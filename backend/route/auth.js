...
router.post('/register', async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: result.insertId, role: 'patient', name, email }, SECRET, { expiresIn: '7d' });
...
router.get('/me', require('../middleware/auth').auth, async (req, res) => {
  try {
    const [users] = await db.execute('SELECT id, name, email, role, phone, avatar, created_at FROM users WHERE id = ?', [req.user.id]);
    res.json(users[0]);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
