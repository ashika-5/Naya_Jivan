...
router.get("/stats", async (req, res) => {
  try {
    ...
    const [[{ totalDoctors }]] = await db.execute(
      "SELECT COUNT(*) as totalDoctors FROM doctors WHERE is_active = TRUE",
    );
    const [[{ totalHospitals }]] = await db.execute(
...
router.post("/hospitals", async (req, res) => {
  try {
    const { name, address, city, phone, email, description, specialties } =
      req.body;
...
router.delete("/doctors/:id", async (req, res) => {
  try {
    await db.execute("UPDATE doctors SET is_active = FALSE WHERE id = ?", [
      req.params.id,
    ]);
